...
class C51:
    ...
    def fit(self, gamma, delta_z, z, v_min, v_max, states, actions, rewards, obs_states, dones):
        ...
        with tf.GradientTape() as tape:
            # Get target qvalues
            p_target = np.array(self.model_tg(obs_states))
            #obs_qvalues_target = tf.math.multiply(p_target, z)
            #obs_qvalues_target = tf.math.reduce_sum(obs_qvalues_target, axis=-1)
            #print(obs_qvalues_target)
            #print(obs_qvalues_target.shape)

            # Get obs_actions
            obs_p = self.model(obs_states)
            obs_qvalues = tf.math.multiply(obs_p, z)
            obs_qvalues = tf.math.reduce_sum(obs_qvalues, axis=-1)
            obs_action = tf.math.argmax(obs_qvalues).numpy()
            #print(obs_action.shape)
           
            # Compute the projection Tz onto the support z
            # First we want to 0 the z where done, as the distribution collapses to a single point
            z_dones = z * dones
            z_ = np.ones((len(rewards), 1)) * z_dones # z is (1, n_atoms), multiplied by (batch_size, 1), we can a matrix (batch_size, n_atoms), with z on each row
            z_ *= gamma

            rewards_ = np.ones((1, z.shape[1])) * rewards # rewards are (batch_size, 1), multiplied by (1, n_atoms), we can a matrix (batch_size, n_atoms), with rewards on each column
         
            Tz = np.minimum(v_max, np.maximum(v_min, rewards_ + z_))
            ...
            m_prob = np.zeros((self.env.action_space.n, len(rewards), z.shape[1]))
            for i in range(64):
                for j in range(51):
                    if dones[i][0] == 0:
                        m_prob[actions[i]][i][int(l[i][j])] += u[i][j] - bj[i][j]
                        m_prob[actions[i]][i][int(u[i][j])] += bj[i][j] - l[i][j]
                    else:
                        m_prob[actions[i]][i][int(l[i][j])] += (u[i][j] - bj[i][j]) * p_target[obs_action[i]][i][j]
                        m_prob[actions[i]][i][int(u[i][j])] += (bj[i][j] - l[i][j]) * p_target[obs_action[i]][i][j]
            
            
            y_pred = self.model(states)
            loss = tf.keras.losses.categorical_crossentropy(m_prob, y_pred)
